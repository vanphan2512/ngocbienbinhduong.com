...
class WC_Admin_Post_Types {
	...
	public function shop_order_columns( $existing_columns ) {
		...
		$columns['view_short_order']      = __( 'Packing Slip', 'woocommerce' );
	...
	public function render_shop_order_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'view_short_order':
				echo '<a href="#TB_inline?width=100&height=100%&inlineId=modal-window-id-'.$post->ID.'&modal=false" class="thickbox">View</a>';
				echo ' | <a href="javascript:void(0)" onClick="printOrderForm('.$post->ID.')">Print</a>';

				$items = $the_order->get_items();
				foreach ($items as $item) {
					$strItem .= '<table width="100%" border="0" cellspacing="0" cellpadding="4" class="detail-product">
					<tr>
						<th>'.$item['name'].'</th>
					</tr>';
					$mtDesign = unserialize(get_post_meta($item['product_id'], 'design', true));
					$custom_fields = get_post_custom($item['product_id']);
					$my_custom_field = $custom_fields['parent_product_id'];
					if (null != $mtDesign) {
						$strDesign = '';
						$strDesign .= 'Product: '.get_the_title($my_custom_field[0]).'</br>';
						$strDesign .= 'ID: '.get_post_meta($item['product_id'], 'parent_product_id', true ).'</br>';
						$strDesign .= 'Fabric Code: '.get_post_meta($item['product_id'], 'fabric_code', true ).'</br>';
						foreach ($mtDesign as $key => $value) {
							$strDesign .= ucfirst($key).': <b>'.$value.'</b><br />';
						}
						$strItem .= '<tr>
							<td><b>Design</b><br />'.$strDesign.'</td>
						</tr>';
					}
					$mtExtra = unserialize(get_post_meta($item['product_id'], 'extras', true));
					if (null != $mtExtra) {
						$strExtra = '';
						foreach ($mtExtra as $key => $value) {
							$strExtra .= ucfirst($key).': <b>'.$value.'</b><br />';
						}
						$strItem .= '<tr>
							<td><b>Extra</b><br />'.$strExtra.'</td>
						</tr>';
					}
					$measureBody = get_post_meta($item['product_id'], 'measure_your_body', true);
					if (null != $measureBody) {
						$strMeasureBody = '';
						$general = $measureBody['general'];
						unset($measureBody['general']);
						foreach ($measureBody as $key => $value) {
							$strMeasureBody .= ucfirst($key).': <b>'.$value.'</b><br />';
						}
						$strMeasureBody .= '<br />';
						...
						$strItem .= '<tr>
							<td>Measurement Type:<b> Body</b><br />'.$strMeasureBody.'</td>
						</tr>';
					}
                ...
                foreach($customer_orders as $k => $v)
				{
				    ...
				    foreach ($items as $item) {
				    	if (is_array($history_products[$item['name']])) {
				    		$history_products[$item['name']] = array_merge($history_products[$item['name']], array($item['product_id']));
				    	} else {
				    		$history_products[$item['name']] = array($item['product_id']);
				    	}
				    }
				}
				$strName = $strID = '';
				...
				$html = '<div id="modal-window-id-'.$post->ID.'" style="display:none;">';
				$html .= '<p>Order '.$the_order->get_order_number().'</p>';
				$html .= '<table width="100%" border="0" cellspacing="0" cellpadding="4" >
				<tr>
					<th>Shipping Details</th>
				</tr>
				<tr>
					<td>'.$the_order->get_formatted_billing_address().'</td>
				</tr>
				</table>';
				$html .= '<p>Nr.Of Items: '.$the_order->get_item_count().'</p>';
				$html .= '<table width="100%" border="0" cellspacing="0" cellpadding="4" class="ship-method">
				<tr>
					<th>Shipping Method</th>
					<th class="col-center">Order History</th>
					<th>Label</th>
				</tr>
				<tr>
					<td>'.$the_order->get_shipping_method().'</td>
					<td class="col-center" >                                                      
						<table cellspacing="0" cellpadding="1" class="tb-order-history">
							'.$str.'
						</table>
					</td>
					<td> J.Coutier </td>
