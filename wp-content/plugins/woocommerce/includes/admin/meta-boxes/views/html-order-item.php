...
<tr>
	<td colspan="4" class="wc-order-edit-line-item"><a onclick="show_hide('<?php echo $_product->id; ?>');" href="javascript:void(0);">Show Details</a></td>
</tr>
<tr class="show_hide">
								
								<td colspan="5" id="product-detail-<?php echo $_product->id; ?>"  style="display:none">
								
									<div class="product-detail-<?php echo $_product->id; ?>">
									...
									<?php
									if (metadata_exists ( 'post', $_product->id, 'design' ))	{
									$design_meta = unserialize(get_post_meta($_product->id,'design' , true));
								...
								<div class="col">
								<?php
								$ext_meta = unserialize(get_post_meta($_product->id,'extras' , true));
								if(count($ext_meta)> 0)
								{
								?>
								<h3>Extras</h3>
								...
								<?php
									if (metadata_exists ( 'post', $_product->id, 'extras' ))	{
									$design_meta = unserialize(get_post_meta($_product->id,'extras' , true));
								...
								<div class="col">
									...
									$measurement=unserialize($measure);
										...
										if (metadata_exists ( 'post', $_product->id, 'measure_your_body' )){
												...
												foreach($measurement['general'] as $selected_key=>$selected_values_option)
												{
													if(!empty($selected_values_option))
													{
														if($selected_key!='general')
