					...
					foreach( $wishlist as $values ) :
						...
						if( $product_obj !== false && $product_obj->exists() ) : ?>
							<tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
								...
								<?php endif ?>
									<td class="product-remove"><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'mr_tailor' ) ?>"><i class="getbowtied-icon-close_regular"></i></a></td>
							</tr>
						<?php
						endif;
					endforeach;
